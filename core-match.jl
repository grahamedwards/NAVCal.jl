...
md" Zoom: 0% $\longleftrightarrow$ 100%"
...
md"Basal core age (NAVC varve year)"
...
let
	...
	if normalized
		ycore = copy(coredata.y) .- (sum(coredata.y)/length(coredata.y))
		ynavc = copy(navcdata.z) .- (sum(navcdata.z)/length(navcdata.z))
	end
	...
	Makie.lines!(ax, xnavc, ynavc)

	if !isnothing(coredata)
		Makie.lines!(ax, coredata.x .+ (corestart-1), coreamp .* ycore)
	end
	f
end |> WideCell
